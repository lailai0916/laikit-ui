...
import TitleCard from '../TitleCard/index.js';
import Skeleton from '../Skeleton/index.js';
import DataState from '../DataState/index.js';
...
function sliceColor(i: number, n: number): string {
  ...
  return `color-mix(in srgb, var(--lk-color-primary) ${strength}%, var(--lk-card-background-color))`;
}
...
export default function Donut({
  ...
  const { locale, messages } = useLaikit();
  ...
  if (otherY > 0) {
    slices.push({
      key: '__other__',
      y: otherY,
      label: otherLabel ?? messages.other,
      color: 'var(--lk-color-emphasis-400)',
    });
  }
